...
- 現行の `ttf` / `NotoSansJP` 中心の短期 contract は、product surface としては再検討余地がある
...
- 次の実装に入る前に、font support は「ファイル形式ベース」ではなく「user-facing source contract ベース」で再整理する
...
- font support policy は再検討対象。Google Fonts を公式サポート対象に寄せる案を先に思い出す
