...
- `387dc175` `feat(cli): add doctor diagnostics command`
...
| Phase 2B | 進行中 | `doctor` の env / input / fonts / runtime diagnostics を実装し、font policy revisit を残す |
...
現時点では env 診断、`doctor <job-or-template>`、`doctor fonts <job-or-template>`、generate 相当の output runtime/path 診断まで実装した。残論点は font policy revisit と source matrix の固定。
...
1. font support policy を「ファイル形式ベース」から「source contract ベース」に再整理する
2. `doctor` / `generate` の font source matrix test を増やす
3. output dir / permission 系の runtime matrix test を増やす
...
- `packages/cli` test は 85 件 green、lint も green
...
- Next priority は font support policy revisit と font source matrix の固定
