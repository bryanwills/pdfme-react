...
- `36d4ee75` `feat(cli): add explicit font source contract`
...
現時点では env 診断、`doctor <job-or-template>`、`doctor fonts <job-or-template>`、generate 相当の output runtime/path 診断、font source contract の first pass を実装し、runtime permission matrix と font edge-case matrix も test で固定した。残論点は Google Fonts / remote font policy の product 判断。
...
1. Google Fonts / remote font policy を product surface として再評価する
2. 上記方針に合わせて `doctor` / `generate` / README の warning vs blocking contract を再整理する
...
- `packages/cli` test は 104 件 green、lint も green
...
- Next priority は Google Fonts / remote font policy の再評価
