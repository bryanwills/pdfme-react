...
次の残タスクは Phase 2B (`doctor`) の再評価と、font policy revisit を product spec として詰めること。
...
1. Phase 2B として `doctor` の最小 contract を具体化する
2. font support policy を「ファイル形式ベース」から「source contract ベース」に再整理する
3. rich text / markdown track は引き続き別トラックで spec 先行にする
...
- `packages/cli` test は 74 件 green、lint も green
