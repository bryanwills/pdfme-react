...
- `c8f43338` `feat(cli): expand doctor runtime diagnostics`
...
現時点では env 診断、`doctor <job-or-template>`、`doctor fonts <job-or-template>`、generate 相当の output runtime/path 診断、font source contract の first pass まで実装した。残論点は runtime permission matrix と font edge-case matrix の固定。
...
1. output dir / permission 系の runtime matrix test を増やす
2. `doctor` / `generate` の font source edge-case matrix を増やす
3. Google Fonts / remote font policy を product surface として再評価する
...
- `packages/cli` test は 93 件 green、lint も green
...
- Next priority は runtime permission matrix と font source edge-case matrix の固定
