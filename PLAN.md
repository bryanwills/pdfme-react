...
- `7ea06d0a` `Simplify CLI examples manifest loading`
...
| Phase 2A | 進行中 | `@pdfme/cli` の contract hardening |
| Phase 2B | 未着手 | `doctor` など運用向け UX を追加検討 |
...
- 短期的な強保証は `ttf` と `NotoSansJP`
- `otf` / `ttc` は当面 unsupported error に寄せる
...
- [ ] manifest に載る official examples はすべて CLI-supported である contract を metadata / test で固定する

### Track C: examples metadata の contract 明示化

- [ ] official examples の CLI 対応範囲を manifest / metadata で表現する項目を定義する
- [ ] metadata から official examples contract を判定できるようにする
- [ ] examples metadata の shape を test で固定し、意図しない変更を検出できるようにする
- [ ] `examples` が current official manifest を deterministic に利用できる前提を崩さないことを確認する

### Track D: `validate` 出力拡張

- [ ] `inspect` を新設せず、`validate --json` で inspection needs を吸収する方針を実装に落とす
- [ ] `validate --json` の success payload に pages, fields, schema types, required fonts/plugins, basePdf summary を追加する
- [ ] template-only / unified job / stdin / `--strict` の各入力経路で同じ contract を返すようにする
- [ ] unsupported font / plugin / malformed input を internal crash ではなく structured error で返すことを test で固定する
- [ ] success / failure envelope を他 command と同様に parse 可能な JSON contract として維持する

### Track E: Phase 2A 完了判定

- [ ] Exit Criteria 1: official examples の CLI 対応範囲が manifest / metadata で定義されていることを確認する
- [ ] Exit Criteria 2: CLI 対応 examples が CI で real CLI path generate green であることを確認する
- [ ] Exit Criteria 3: `examples` が current official manifest を deterministic に取得・利用できることを確認する
- [ ] Exit Criteria 4: `--json` 指定時に全対象 command の成功失敗が JSON で parse 可能であることを確認する
- [ ] Exit Criteria 5: invalid args / invalid enum / invalid number / invalid page range が fail-fast + non-zero であることを確認する
- [ ] Exit Criteria 6: `validate` が template-only / unified job / stdin / `--strict` を一貫処理することを確認する
- [ ] Exit Criteria 7: font / plugin / malformed input が structured error になることを確認する
- [ ] Exit Criteria 8: `generate` / `validate` / `pdf2img` / `pdf2size` が examples 取得と独立に offline で成立することを確認する
- [ ] Exit Criteria 9: workspace 内でも user-facing version 表示が正しいことを確認する
- [ ] Phase 2A 完了までは `doctor` に着手しない
