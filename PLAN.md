...
- `9aa7dcf3` `Refactor CLI schema plugin discovery`

この文書は `PROGRESS.md` と `REPORT.md` を統合した、単一の運用ドキュメントとする。
...
- official examples を real CLI path ベースで全件 green にする
...
- `pdfme doctor`
  ...
  - examples cache dir / temp dir / output dir の状態
...
3. examples fetch / cache / versioning を CI gate 化する
4. `validate` の出力拡張で inspection needs を吸収する
5. Phase 2A 完了後に `doctor` を再評価する
