...
- Phase 2B は feature expansion の段階ではなく、scope judgment と contract closeout の段階に入っている
- fixed decision と remote font の failure contract は固定済みで、残作業は closeout 更新に絞られている
...
1. Phase 2B の closeout 条件を更新する
   - `doctor` / `generate` / `validate` の font diagnosis と error code の整合性を確認する
   - Phase 2B を「remote font policy first pass と operational UX の固定完了」として閉じる条件を書く
   - closeout 後の次トラックを Rich Text / Markdown spec に切り分ける
2. docs / README / release note 反映
...
- `--verbose` の option parity は `pdf2img` だけでなく `pdf2size` も含めて見直し、command 間での期待を揃える
...
- `multiVariableText` failure では、expected variable names と JSON string 例を含む error message に寄せる
- `doctor` / `validate --json` が field ごとの期待入力形式を返せるようにし、`text` と `multiVariableText` の違いを事前発見できるようにする
...
- `packages/cli` test は 111 件 green、lint も green
...
- `multiVariableText` の expected input discoverability は次の UX 改善候補として保持する
