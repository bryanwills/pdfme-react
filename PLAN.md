...
- `6ff816bc` `test(cli): ignore generated thumbnail assets in CI`
...
| Phase 2B | 進行中 | `doctor` の最小 contract を実装し、残りを段階的に詰める |
...
次の残タスクは Phase 2B (`doctor`) の深掘りと、font policy revisit を product spec として詰めること。
...
Phase 2A 完了後の first slice として `pdfme doctor` の最小 contract を追加した。
現時点では env 診断と `doctor <job-or-template>` のみ実装し、`doctor fonts` は未着手。
...
1. `doctor fonts <job-or-template>` 相当の font source 診断を追加する
2. `doctor` の runtime/path 診断を output dir などまで広げる
3. font support policy を「ファイル形式ベース」から「source contract ベース」に再整理する
...
- `doctor` の minimal contract を追加し、env 診断と input 診断を `--json` で返せる
- `packages/cli` test は 78 件 green、lint も green
...
- Next priority は `doctor` の contract 設計と font support policy revisit
